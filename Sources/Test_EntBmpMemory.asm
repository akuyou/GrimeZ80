...
ifndef ScrWid256
	...
	GetNextLine:
	push af
		ld hl,&0000:ScreenLinePos_Plus2
		ld a,&50
		add l
		ld l,a
		...
		inc h
;		push bc
;			ld bc,&0050	;if we got here we need to jump back to the top of the screen - the command here will do that
;			add hl,bc
;		pop bc
	GetNextLineDone:
	pop af
...
VIDADR1:   DW 0    ;primary video address, address of pixel data
   ...
   DB 36,36,219,73,0,0,0,0 ;black, blue, yellow, red (not used colour are black)  g0 | r0 | b0 | g1 | r1 | b1 | g2 | r2 | - x0 = lsb 

   DB -20
   db %00110010;	db %01010010 16 color ver
   DB 11   ;left margin=15
   DB 51   ;right margin=47
   DW 80*25*1    ;primary video address, address of pixel data
   DW 0    ;secondary vide・address, not used in pixel graphics mode
   DB 73,216,36,0,0,0,0,0 ;black, blue, yellow, red (not used colour are black)  g0 | r0 | b0 | g1 | r1 | b1 | g2 | r2 | - x0 = lsb 

   DB -20
   db %00110010;	db %01010010 16 color ver
   DB 11   ;left margin=15
   DB 51   ;right margin=47
   DW 0    ;primary video address, address of pixel data
   DW 0    ;secondary vide・address, not used in pixel graphics mode
   DB 0,36,219,73,0,0,0,0 ;black, blue, yellow, red (not used colour are black)  g0 | r0 | b0 | g1 | r1 | b1 | g2 | r2 | - x0 = lsb 
